...
CREATE POLICY "Allow users to insert their own profile" ON users FOR INSERT WITH CHECK (auth.uid()::text = id::text);
CREATE POLICY "Allow authenticated users to insert profiles" ON users FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Allow service role to manage all users" ON users FOR ALL USING (auth.role() = 'service_role');
