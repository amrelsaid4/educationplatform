-- Fix Student Dashboard Database Issues
...
INSERT INTO student_achievements (user_id, achievement_type, achievement_data) 
SELECT 
  u.id,
  'first_course',
...
FROM auth.users u
WHERE u.id NOT IN (
  SELECT user_id FROM student_achievements WHERE achievement_type = 'first_course'
)
LIMIT 5;

-- Insert sample assignments for testing
INSERT INTO assignments (title, description, course_id, teacher_id, assignment_type, max_score, due_date, instructions, is_published)
SELECT 
  'واجب الدرس الأول',
  'قم بحل التمارين المرفقة في نهاية الدرس',
  c.id,
  c.teacher_id,
  'homework',
  100,
  NOW() + INTERVAL '7 days',
  'اقرأ الدرس بعناية ثم حل التمارين',
  true
FROM courses c
WHERE c.id NOT IN (SELECT course_id FROM assignments WHERE title = 'واجب الدرس الأول')
LIMIT 3;

-- Insert sample exams for testing
INSERT INTO exams (title, description, course_id, teacher_id, duration_minutes, total_questions, passing_score, is_published, start_date, end_date)
SELECT 
  'امتحان منتصف الفصل',
  'امتحان شامل على المحتوى المغطى حتى الآن',
  c.id,
  c.teacher_id,
  60,
  20,
...
FROM courses c
WHERE c.id NOT IN (SELECT course_id FROM exams WHERE title = 'امتحان منتصف الفصل')
LIMIT 3;
